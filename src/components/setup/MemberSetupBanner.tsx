...
interface MemberSetupBannerProps {
  onOpenWizard: () => void;
}

export function MemberSetupBanner({ onOpenWizard }: MemberSetupBannerProps) {
  const { loading, isComplete, steps, completionPercentage } = useMemberSetupStatus();
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-foreground">Configure seu perfil</h3>
                <p className="text-xs text-muted-foreground">Complete suas informações para começar a usar</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {steps.map((step) => (
                ...
                >
                  {step.isComplete ? <CheckCircle className="w-2.5 h-2.5" /> : <Circle className="w-2.5 h-2.5" />}
                  {step.title}
                </div>
              ))}
            </div>
